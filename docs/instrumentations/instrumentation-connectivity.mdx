...
스키마는 JSON 문서로, JSON 스키마 사양을 따릅니다. 연결성 테이lemetry 데이터의 구조화된 형식을 정의하며, 다음과 같은 주요 구성요소를 포함합니다:
...
이 스키마는 IMQA 모니터링 시스템으로 테이lemetry 데이터를 처리하고 저장하기 전에 연결 상태 변경 테이lemetry 데이터를 검증하는 데 사용됩니다. 이는 모든 필수 필드가 올바르게 포함되어 있는지 확인합니다.

## 예시

유효한 연결성 테이lemetry 객체는 다음과 같은 내용을 포함합니다:

---
title: Instrumentation Longtask
...
브라우저 장시간 작업 스키마는 브라우저에서 실행되는 장시간 작업을 캡처하고 보고하는 구조를 정의합니다. 이 스키마는 OpenTelemetry 형식을 사용하여 웹 애플리케이션의 성능 문제를 추적하도록 설계되었습니다.

## 스키마 구조

스키마는 JSON 문서로, JSON 스키마 사양을 따릅니다. 브라우저 장시간 작업 테이lemetry 데이터의 구조화된 형식을 정의하며, 다음과 같은 주요 구성요소를 포함합니다:
...
각 스팬은 브라우저에서 실행되는 단일 장시간 작업을 나타내며, 다음과 같은 내용을 포함합니다:
...
| longtask.name | string | 장시간 작업의 소스 (예: "self") |
| longtask.entry_type | string | 엔트리 타입 (일반적으로 "longtask") |
| longtask.duration | integer | 장시간 작업의 지속 시간 (밀리초) |
...
이 스키마는 IMQA 모니터링 시스템으로 테이lemetry 데이터를 처리하고 저장하기 전에 장시간 작업 테이lemetry 데이터를 검증하는 데 사용됩니다. 이는 모든 필수 필드가 올바르게 포함되어 있는지 확인합니다.

## 예시

유효한 브라우저 장시간 작업 테이lemetry 객체는 다음과 같은 내용을 포함합니다:

- 서비스, 브라우저 및 환경을 식별하는 리소스 정보
- 장시간 작업을 나타내는 하나 이상의 스팬
- 작업 지속 시간과 속성에 대한 상세 정보를 포함하는 각 스팬

장시간 작업은 50ms 이상 실행되는 작업으로, 사용자 경험 저하를 초래할 수 있습니다. 이러한 작업을 모니터링하면 웹 애플리케이션의 성능 병목 현상을 식별하는 데 도움이 됩니다.

데이터는 다양한 테이lemetry 수집 및 분석 도구와 호환되는 OpenTelemetry 프로토콜 형식을 따릅니다.

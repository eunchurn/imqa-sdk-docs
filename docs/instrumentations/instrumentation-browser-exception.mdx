...
브라우저 예외 스키마는 IMQA 모니터링 시스템에서 웹 애플리케이션의 JavaScript 오류와 예외를 캡처하고 보고하는 구조를 정의합니다. 이 스키마는 OpenTelemetry 형식을 사용하여 클라이언트 측에서 발생하는 런타임 오류, 처리되지 않은 예외, 콘솔 에러 등을 추적하도록 설계되었습니다.

## 스키마 구조

스키마는 JSON 문서로, JSON Schema Draft 2020-12 사양을 따릅니다. 브라우저 예외 Telemetry 데이터의 구조화된 형식을 정의하며, 다음과 같은 주요 구성요소를 포함합니다:

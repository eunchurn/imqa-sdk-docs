...
### IMQAErrorInstrumentationConfig

| 옵션 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `captureConsoleError` | `boolean` | `true` | `console.error` 호출을 exception으로 캡처할지 여부 |
| `applyCustomAttributes` | `(error: Error \| string \| Event) => Attributes` | `undefined` | 에러 발생 시 커스텀 속성을 추가하는 콜백 함수 |

> `screenNameOption`은 모든 계측기에서 공통으로 사용되는 옵션입니다.

### 기본 동작

- `captureConsoleError: true` (기본값): `console.error` 호출이 exception으로 캡처됨
- `captureConsoleError: false`: `console.error` 계측이 비활성화되어 exception으로 캡처되지 않음
- `applyCustomAttributes`: 에러가 기록될 때 span과 log에 커스텀 속성을 추가하는 콜백 함수. 에러 객체, 문자열, 또는 Event를 인자로 받아 추가할 속성 객체를 반환

### 설정 예시

```typescript
import { IMQAErrorInstrumentation } from '@imqa/instrumentation-browser-exception';

// 기본 설정 (console.error 캡처 활성화)
const errorInstrumentation = new IMQAErrorInstrumentation({});

// console.error 캡처 비활성화
const errorInstrumentation = new IMQAErrorInstrumentation({
...
### 커스텀 속성 추가 (applyCustomAttributes)

`applyCustomAttributes` 콜백을 사용하면 에러 발생 시 추가 컨텍스트를 span과 log에 첨부할 수 있습니다:
    ...
    errors: {
      applyCustomAttributes: (error) => {
        ...
        if (error instanceof Error) {
          ...
          switch (error.name) {
            case 'TypeError':
              attributes['error.category'] = 'type-error';
              break;
            case 'ReferenceError':
              attributes['error.category'] = 'reference-error';
              break;
            case 'NetworkError':
              attributes['error.category'] = 'network-error';
              break;
            default:
              attributes['error.category'] = 'general';
          }

          // 에러 메시지에서 추가 정보 추출
          if (error.message.includes('fetch')) {
            attributes['error.source'] = 'api-call';
          }
        }
        ...
        if (error instanceof Event && error.target) {
          ...
          attributes['error.category'] = 'resource-load-error';
          attributes['error.element'] = target.tagName?.toLowerCase() || 'unknown';
        }

        // 공통 속성
        attributes['error.timestamp'] = new Date().toISOString();
        attributes['error.page'] = window.location.pathname;

...
문서 로드 후 계측은 웹 페이지의 초기 로딩 완료 이후에 발생하는 지연 로딩 및 동적 콘텐츠 로딩을 추적하는 기능을 제공합니다. 지연 로딩 이미지, 동적 스크립트, AJAX 콘텐츠 등 사용자 상호작용에 따라 추가로 로딩되는 리소스들을 모니터링할 수 있습니다.

## 주요 기능

### 지연 로딩 추적
- Lazy loading 이미지 및 콘텐츠 모니터링
- Intersection Observer 기반 로딩 추적
- 사용자 스크롤에 따른 콘텐츠 로딩

### 동적 리소스 모니터링
- 동적으로 생성되는 스크립트 및 스타일시트
- AJAX로 로딩되는 콘텐츠 추적
- 사용자 액션에 따른 리소스 로딩
...
## 주요 속성description: '@imqa/instrumentation-post-doc-load-resource'
nav: 1018
---

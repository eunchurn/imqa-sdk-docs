---
title: Long Task 계측
description: '@imqa/instrumentation-browser-longtask'
nav: 1015
---
...
## 주요 속성cription: '@imqa/instrumentation-browser-longtask'
nav: 1015
---
...
브라우저 Long Task 스키마는 IMQA 모니터링 시스템에서 브라우저의 메인 스레드를 50ms 이상 차단하는 긴 작업을 캡처하고 보고하는 구조를 정의합니다. 이 스키마는 OpenTelemetry 형식을 사용하여 웹 애플리케이션의 성능 병목 현상과 사용자 경험 저하를 야기할 수 있는 장기 실행 작업을 추적하도록 설계되었습니다.

## 스키마 구조

스키마는 JSON 문서로, JSON Schema Draft 2020-12 사양을 따릅니다. 브라우저 Long Task Telemetry 데이터의 구조화된 형식을 정의하며, 다음과 같은 주요 구성요소를 포함합니다:
...
브라우저 Long Task 계측을 활성화하거나 비활성화합니다. `true`로 설정하면 50ms 이상 실행되는 긴 작업이 자동으로 계측됩니다.

...
WebVitals 스키마는 IMQA 모니터링 시스템에서 웹 성능 메트릭을 캡처하는 구조를 정의합니다. 이 스키마는 OpenTelemetry 형식을 사용하여 웹 애플리케이션의 핵심 성능 지표를 추적하도록 설계되었습니다.
...
- `endTimeUnixNano`: 에포치 이후의 종료 시간 (나노초)
- `status`: 결과 상태 (0 = OK)

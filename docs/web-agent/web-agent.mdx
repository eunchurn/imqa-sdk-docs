...
- `os.name`: 운영체제 이름
- `os.version`: 운영체제 버전
- `telemetry.sdk.name`: SDK 이름
- `telemetry.sdk.version`: SDK 버전
- `telemetry.sdk.language`: SDK 언어
...
- `device.model.name`: 디바이스 모델 이름 (nullable)
- `user_agent.original`: User Agent 문자열
- `service.key`: 서비스 키
- `network.carrier.name`: 통신사 이름 (nullable)
- `network.connection.subtype`: 네트워크 연결 유형, 통신 방식 (nullable)
- `device.battery.status`: 배터리 상태 (nullable)
- `device.cpu.util`: CPU 사용률 (nullable)
- `device.memory.util`: 메모리 사용률 (nullable)
...
- `screen.name`: 현 스팬의 화면 이름
- `screen.type`: 화면 타입 (e.g., "route", "modal", "dialog")

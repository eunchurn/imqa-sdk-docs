...
| collectSessionId           | `boolean`                                                                                                                                                 | `true`       | `rum.session_id` 속성 수집 활성화                                                           |
...
여러 출처 서비스의 유효한 구성을 IMQA는 수행할 수 없으므로, 이 설정을 할 때는 주의하십시오. 프로덕션 설정 전에 테스트 환경에서 CORS를 활성화하는 것이 좋습니다.
...
- [OpenTelemetry in JavaScript](https://scoutapm.com/blog/opentelemetry-in-javascript)
...
- `api` - [@opentelemetry/api](https://www.npmjs.com/package/@opentelemetry/api) 모듈 노출
- `tracer` - [@opentelemetry/tracing](https://www.npmjs.com/package/@opentelemetry/tracing)에서 가져온 `Tracer`의 인스턴스

...
IMQA Web Agent의 초기화 옵션은 다음 매개변수를 사용하여 구성할 수 있습니다:
...
| samplingProbability | `number \| string` | `1` | `1`은 모든 추적이 전송됨을, `0`은 추적이 전송되지 않음을, `0.5`는 50%의 확률로 추적이 전송됨을 의미 |
| bufferMaxSpans | `number` | `2048` | 전송 대기 중인 최대 스팬 수 |
| maxExportBatchSize | `number` | `50` | 한 번의 요청에 포함될 최대 스팬 수 |
| bufferTimeout | `number` | `2000` | 전송 대기 중인 스팬의 시간 (밀리초) |
...
| collectErrors | `object \| undefined` | `{ error: true, typeError: true, syntaxError: true, referenceError: true }` | 에러 수집 설정 (설정하지 않은 경우 모두 수집) |
| collectErrors.error | `boolean \| undefined` | `true` | 일반 에러 수집 여부 |
| collectErrors.typeError | `boolean \| undefined` | `true` | TypeError 수집 여부 |
| collectErrors.syntaxError | `boolean \| undefined` | `true` | SyntaxError 수집 여부 |
| collectErrors.referenceError | `boolean \| undefined` | `true` | ReferenceError 수집 여부 |
| collectLogs | `object \| undefined` | `{ log: true, info: true, debug: true, warn: true, error: true }` | 로그 수집 설정 (설정하지 않은 경우 모두 수집) |
| collectLogs.log | `boolean \| undefined` | `true` | console.log 수집 여부 |
| collectLogs.info | `boolean \| undefined` | `true` | console.info 수집 여부 |
| collectLogs.debug | `boolean \| undefined` | `true` | console.debug 수집 여부 |
| collectLogs.warn | `boolean \| undefined` | `true` | console.warn 수집 여부 |
| collectLogs.error | `boolean \| undefined` | `true` | console.error 수집 여부 |
...
| collectWebVitals | `boolean` | `false` | 웹 바이탈 메트릭 수집 유무 |
| webVitalsExportIntervalMillis | `number` | `10000` | 웹 바이탈 메트릭의 데이터 수집 주기 (밀리초) |

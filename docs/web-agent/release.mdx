---
title: Release Notes
description: Release notes for @imqa/web-agent
nav: 302
pdf: true
cover: web
release: https://api.github.com/repos/idlerecord/imqa-js/releases
---

## Latest Release
![NPM Version](https://img.shields.io/npm/v/%40imqa%2Fweb-agent?style=flat-square&logo=npm&color=%238ef7c1) ![NPM Last Update](https://img.shields.io/npm/last-update/%40imqa%2Fweb-agent?color=%238ef7c1) ![NPM Unpacked Size](https://img.shields.io/npm/unpacked-size/%40imqa%2Fweb-agent?color=%238ef7c1) ![npm package minimized gzipped size](https://img.shields.io/bundlejs/size/%40imqa%2Fweb-agent?color=%238ef7c1) ![NPM Downloads](https://img.shields.io/npm/dw/%40imqa%2Fweb-agent?color=%238ef7c1) ![NPM Type Definitions](https://img.shields.io/npm/types/%40imqa%2Fweb-agent)

[![JSON SBOM](https://img.shields.io/badge/SBOM-JSON-blue)](https://cdn.imqa.io/release/npm/@web-agent/latest/cyclonedx/bom.json)
[![XML SBOM](https://img.shields.io/badge/SBOM-XML-orange)](https://cdn.imqa.io/release/npm/@web-agent/latest/cyclonedx/bom.xml)
[![Well-Known SBOM](https://img.shields.io/badge/SBOM-.well--known-green)](https://cdn.imqa.io/release/npm/@web-agent/latest/.well-known/sbom)

## Release Notes
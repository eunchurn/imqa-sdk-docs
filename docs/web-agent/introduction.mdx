...
화면 이름을 설정합니다. IMQA는 화면 이름을 자동으로 수집합니다. 그러나 특정 화면에 대한 추가 정보를 제공하고 싶다면 `setScreenAttribute` 함수를 사용하여 화면 속성을 설정할 수 있습니다. 이 함수는 화면 이름을 설정하고, 현재 URL을 전역 변수에 기록합니다. 이 매서드는 다이내믹 메서드입니다. 즉, URL(`location.href`)이 변경되지 않는 한 이 메서드로 설정한 화면 이름으로 페이지 성능을 계측하게 됩니다. URL이 변경되면 화면 옵션(`screenNameOption`)에 따라 화면 이름이 자동으로 리셋됩니다. 따라서 지속적으로 화면 이름을 유지하려면 URL이 변경되거나 화면이 변경되는 즉시 이 메서드를 호출해야 효율적인 페이지의 성능을 계측합니다.

계측된 화면별 성능 데이터는 IMQA 화면 분석화면에서 유니크한 화면 이름으로 활용합니다. 자세한 내용은 **사용자 가이드**의 화면 분석 섹션을 참고하세요.

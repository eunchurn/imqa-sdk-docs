...
- `proto`로 설정할 경우 `protobuf` 프로토콜을 사용하여 프로토 형식(`application/x-protobuf`)로 데이터를 전송하며, [Fetch API](https://developer.mozilla.org/ko/docs/Web/API/Fetch_API)통해 전송합니다.
...
<ul>
  <li>1. 페이지 언로드 시에도 데이터 전송 보장: 사용자가 페이지를 닫거나 이동할 때, 일반적인 fetch/XMLHttpRequest는 네트워크 요청이 중단될 수 있습니다. `sendBeacon`은 브라우저가 백그라운드에서 전송을 끝까지 시도합니다.</li>
  <li>2. 비동기적이고, 빠름: 로그, 분석, 트레이스, 세션 종료 이벤트 등 "꼭 서버로 보내야 하는 데이터"에 적합합니다.</li>
  <li>3. CORS 지원: Beacon API는 CORS(Cross-Origin Resource Sharing)를 지원하여, 다른 도메인 간의 요청을 처리할 수 있습니다.</li>
  <li>4. 데이터 크기 제한: Beacon API는 전송할 수 있는 데이터의 크기에 제한이 있습니다. 일반적으로 64KB 이하의 데이터를 전송할 수 있습니다.</li>
  <li>5. 브라우저 호환성: 대부분의 최신 브라우저에서 지원되지만, 일부 오래된 브라우저에서는 지원하지 않을 수 있습니다.</li>
  <li>6. 단방향 통신: Beacon API는 서버로 데이터를 전송하는 단방향 통신 방식입니다. 서버에서 응답을 받을 수 없습니다.</li>
  <li>7. 형식제한: Beacon API는 JSON, Text 형식의 데이터를 전송합니다. 따라서, 다른 `Content-Type`은 지원하지 않습니다.</li>
</ul>
</div>
<h4>Fetch API</h4>
<div className='my-4'>
...
<ul>
  <li>1. 더 많은 기능: Fetch API는 HTTP 요청에 대한 더 많은 제어를 제공합니다. 예를 들어, 헤더 설정, 요청 메소드 변경, 응답 처리 등 다양한 기능을 지원합니다.</li>
  <li>2. Promise 기반: Fetch API는 Promise를 반환하므로, 비동기 처리를 더 쉽게 할 수 있습니다.</li>
  <li>3. CORS 지원: Fetch API는 CORS(Cross-Origin Resource Sharing)를 지원하여, 다른 도메인 간의 요청을 처리할 수 있습니다.</li>
  <li>4. 데이터 크기 제한 없음: Fetch API는 전송할 수 있는 데이터의 크기에 제한이 없습니다. 따라서, 대용량 데이터를 전송할 때 유용합니다.</li>
  <li>5. 브라우저 호환성: Fetch API는 대부분의 최신 브라우저에서 지원되지만, 일부 오래된 브라우저에서는 지원하지 않을 수 있습니다.</li>
  <li>6. 양방향 통신: Fetch API는 서버로 데이터를 전송하고, 서버에서 응답을 받을 수 있는 양방향 통신 방식입니다.</li>
  <li>7. 형식제한 없음: Fetch API는 JSON, Text, Blob 등 다양한 형식의 데이터를 전송할 수 있습니다. 따라서, `Content-Type`에 대한 제한이 없습니다.</li>
</ul>
</div>
</Gha>

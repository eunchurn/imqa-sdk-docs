...
pdf: true
cover: ios
release: https://api.github.com/repos/idlerecord/imqa-ios-sdk/releases
---
...
IMQA.setWebviewConfiguration(userContentController: wkWebView.configuration.userContentController)
...
```c
IMQAEndpoits *endpoints = [[IMQAEndpoits alloc] initWithCollectorURL:@"collector 주소"];
IMQAOptions *options = [[IMQAOptions alloc] initWithServiceKey:@"서비스 키" endpoints:endpoints sampleRate:1.0];
NSError *setUpError = nil;
IMQA *imqa = [IMQA setupWithOptions:options error:&setUpError];
if (setUpError) {
    NSLog(@"IMQA setUp 실패: %@", setUpError.localizedDescription);
} else {
    NSLog(@"IMQA setUp 성공: %@", imqa);
}

NSError *startError = nil;
[imqa startAndReturnError:&startError];
if (startError) {
    NSLog(@"IMQA start 실패: %@", startError.localizedDescription);
} else {
    NSLog(@"IMQA 성공");
}
```

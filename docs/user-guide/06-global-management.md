...
> [!INFO] 글로벌 관리 메뉴 접근 권한
> IMQA의 글로벌 관리 메뉴는 `MANAGER` 이상의 권한 사용자에게만 제공됩니다.
>
> - **사용자 관리**: `ADMIN`, `SUPER_USER` 역할 사용자만 접근할 수 있습니다.
> - **서비스 관리**: `MANAGER`, `ADMIN`, `SUPER_USER` 역할 사용자만 접근할 수 있습니다.
...
> [!IMPORTANT] 사용자 권한
> IMQA에는 **서비스 접근 권한**과 **기능 접근 권한**으로 구분된 권한 체계가 있습니다. 서비스 사용자는 소속 팀과 역할을 기반으로 각 권한을 상속받습니다. 팀은 서비스 접근 권한을 사용자에게 부여합니다. 역할은 기능 접근 권한을 사용자에게 부여합니다.
...
> [!INFO] 팀-역할-사용자 권한 요약
> **1. 팀 단위로 서비스 접근 권한을 설정할 수 있습니다.** - ⚠️ 팀 단위에는 기능 접근에 대한 설정 기능을 제공하지 않습니다.
> **2. 역할 단위에는 기능 접근 권한이 사전 정의되어 있습니다.** - ⚠️ 역할 단위에는 서비스 접근에 대한 설정 기능을 제공하지 않습니다. - ⚠️ 현재 역할을 새로 생성하거나 역할별 권한은 수정이 되지 않습니다.
> **3. 사용자 단위로 팀과 역할을 설정할 수 있습니다.** - 사용자는 기본 '(기본 팀)'으로 소속되며, '(기본 팀)'은 모든 서비스의 접근 권한을 가지고 있습니다. - 사용자의 소속 팀을 설정하면 팀에 설정된 서비스 접근 권한을 자동 설정합니다. - 사용자가 복수의 팀에 설정된 경우 각 팀에 설정된 서비스 접근 권한을 병합합니다. - 사용자의 역할을 설정하면 역할에 설정된 기능 접근 권한을 자동 설정합니다.
> **4. 사용자 단위로 소속 팀과 역할에 지정된 권한을 상속받아야 합니다.** - ⚠️ 현재 사용자에게 별도 서비스 접근 권한과 기능 접근 권한을 설정할 수 없습니다.
...
- **역할**: 전체, SUPER_USER, ADMIN 등 역할로 사용자 목록을 필터링합니다.

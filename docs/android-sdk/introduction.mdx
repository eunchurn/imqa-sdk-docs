...
description: IMQA Android SDK v1.0.31 설치 및 설정 방법을 안내합니다.
nav: 410
pdf: true
cover: android
---
...
```groovy
dependencies {
  implementation 'io.imqa:imqa-android-sdk:1.0.31'
}
```
...
```groovy
dependencies {
  implementation files('libs/imqa-android-sdk-1.0.31-release.aar')
}
```
...
```kotlin
// HttpURLConnection 객체를 ConnectionWrapper 로 감쌈
...
```groovy
    compileOnly(files("libs/imqa-android-sdk-1.0.31-release.aar"))
```

...
description: IMQA Android SDK v1.1.10 설치 및 설정 방법을 안내합니다.
nav: 411
pdf: true
cover: android
---
...
```groovy
dependencies {
  implementation 'io.imqa:imqa-android-sdk:1.1.10'
}
```
...
```groovy
dependencies {
  implementation files('libs/imqa-android-sdk-1.1.10-release.aar')
}
```
...
```groovy
dependencies {
  implementation files('libs/imqa-android-sdk-1.1.10-release.aar') //IMQA SDK
...
또한 Text/ID 수집 여부는 옵션으로 제어할 수 있습니다.
해당 옵션은 자동 계측 및 수동 이벤트 모두에 적용되며, OFF 시 모든 이벤트에서 해당 값이 비어있게 됩니다. (개별 View 단위 설정 불가)
...
> options.setUserEventEnable(true);  
> // 추가 옵션   
> options.setUserEventTextEnable(false); // Text 수집 비활성화 (기본값: true)
> options.setUserEventIdEnable(false);   // ID 수집 비활성화 (기본값: true)
...
```groovy
    compileOnly(files("libs/imqa-android-sdk-1.1.10-release.aar"))
```
...
```kotlin
val deviceId = IMQA.getDeviceId()
```

**Java**

```java
String deviceId = IMQA.getDeviceId();
```
...
**setDeviceIdType(deviceIdType: DeviceIdType)**

deviceId를 타입을 설정합니다. 디폴트 값은 UUID 입니다.
- ANDROID_ID : SSAID를 UUID 형식으로 변환
- UUID: SDK 내부에서 생성되는 랜덤 식별자 (앱 삭제 후 재설치시 새로 생성)

 **매개변수:**
- `deviceIdType`: DeviceIdType - ANDROID_ID 또는 UUID

**Kotlin**

```kotlin
option.setDeviceIdType(DeviceIdType.UUID)
```

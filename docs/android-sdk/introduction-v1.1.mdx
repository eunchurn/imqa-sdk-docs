...
description: IMQA Android SDK v1.1.7 설치 및 설정 방법을 안내합니다.
nav: 411
pdf: true
cover: android
---
...
```groovy
dependencies {
  implementation 'io.imqa:imqa-android-sdk:1.1.7'
}
```
...
```groovy
dependencies {
  implementation files('libs/imqa-android-sdk-1.1.7-release.aar')
}
```
...
```groovy
dependencies {
  implementation files('libs/imqa-android-sdk-1.1.7-release.aar') //IMQA SDK
...
```groovy
    compileOnly(files("libs/imqa-android-sdk-1.1.7-release.aar"))
```
...
- ANDROID_ID : 안드로이드에서 제공하는 식별자(SSAID)를 UUID 형식으로 변환

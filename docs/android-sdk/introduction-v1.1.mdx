...
description: IMQA Android SDK v1.1.6 설치 및 설정 방법을 안내합니다.
nav: 411
pdf: true
cover: android
---
...
```groovy
dependencies {
  implementation 'io.imqa:imqa-android-sdk:1.1.6'
}
```
...
```groovy
dependencies {
  implementation files('libs/imqa-android-sdk-1.1.6-release.aar')
}
```
...
```groovy
dependencies {
  implementation files('libs/imqa-android-sdk-1.1.6-release.aar') //IMQA SDK
...
```groovy
    compileOnly(files("libs/imqa-android-sdk-1.1.6-release.aar"))
```
...
deviceId를 타입을 설정합니다. 디폴트 값은 ANDROID_ID 입니다.
